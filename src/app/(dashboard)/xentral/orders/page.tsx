...
type AddressDisplayState = "ok" | "invalid" | "edited";
...
function resolveAddressDisplay(row: {
  addressValidation?: AddressValidationState;
  addressValidationIssues?: string[];
  addressEdited?: boolean;
}): AddressDisplayState {
...
function AddressStatusDisc({ state, title }: { state: AddressDisplayState; title: string }) {
  const cls =
    state === "invalid"
...
function sortAddressDialogOrders(rows: XentralOrderRow[]): XentralOrderRow[] {
...
function applyAddressDemoMerge(items: XentralOrderRow[]): XentralOrderRow[] {
...
function addressFieldNorm(s: string): string {
 ...
 */
function AddressEditStack({
  beforeFrom,
  showGeocodeLoading,
  alignCenter,
  children,
}: {
  /** Gesetzter Wert vor dem Nominatim-Vorschlag; `undefined` = kein Vergleich, nur Platzhalterzeile */
