...
const ZOOPLUS_ORDERS_ACCUMULATED_LS_KEY = "zooplus_orders_accumulated_v1";
...
export default function ZooplusOrdersPage() {
  ...
  const [allRows, setAllRows] = useState<ZooplusOrderRow[]>([]);
  ...
  const allRowsRef = useRef<ZooplusOrderRow[]>([]);
  ...
  const loadOrders = useCallback(
    async (nextFrom?: string, nextTo?: string, forceRefresh = false, silent = false) => {
      ...
      if (!forceRefresh && !silent) {
        const parsed = readLocalJsonCache<CachedOrdersPayload>(ZOOPLUS_ORDERS_ACCUMULATED_LS_KEY);
      ...
      try {
        ...
        setAllRows((prev) => {
          ...
          writeLocalJsonCache(ZOOPLUS_ORDERS_ACCUMULATED_LS_KEY, {
            savedAt: Date.now(),
            items: merged,
    ...
    <div className={DASHBOARD_PAGE_SHELL}>
      ...
      {isLoading && tableRows.length === 0 && allRows.length === 0 ? (
        <div className="rounded-xl border border-border/50 bg-card/80 p-4 text-sm text-muted-foreground">
          {t("ordersShared.loading", { marketplace: t("nav.zooplus") })}
        </div>
      ) : (
        <DataTable
          columns={columns}
