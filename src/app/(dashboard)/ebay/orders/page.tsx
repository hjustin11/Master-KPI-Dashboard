...
const EBAY_ORDERS_ACCUMULATED_LS_KEY = "ebay_orders_accumulated_v1";
...
function useEbayStatusLabel() {
  const { t } = useTranslation();
  return useCallback(
    (raw: string) => {
      const n = raw.trim().toLowerCase();
      if (!n) return t("ebayOrders.statusUnknown");
      if (n.includes("cancel") || n.includes("refund")) return t("ebayOrders.statusCancelled");
      if (
        n.includes("fulfilled") ||
        n.includes("paid") ||
        n.includes("complete") ||
        ...
        return t("ebayOrders.statusCompleted");
      }
      if (n.includes("pending") || n.includes("partial") || n.includes("unpaid") || n.includes("open")) {
        return t("ebayOrders.statusPending");
      }
      return raw;
    },
    [t]
...
export default function EbayOrdersPage() {
  ...
  const labelForStatus = useEbayStatusLabel();
  ...
  const [allRows, setAllRows] = useState<EbayOrderRow[]>([]);
  ...
  const allRowsRef = useRef<EbayOrderRow[]>([]);
  ...
  const columns = useMemo<Array<ColumnDef<EbayOrderRow>>>(
    () => [
      {
        accessorKey: "orderId",
        header: t("ebayOrders.orderId"),
        cell: ({ row }) => (
          <MarketplaceOrderIdLink marketplace="eBay" internetNumber={row.original.orderId} />
        ),
      },
      {
        accessorKey: "purchaseDate",
        header: t("ebayOrders.purchaseDate"),
        cell: ({ row }) => (
  ...
  const loadOrders = useCallback(
    async (nextFrom?: string, nextTo?: string, forceRefresh = false, silent = false) => {
      ...
      if (!forceRefresh && !silent) {
        const parsed = readLocalJsonCache<CachedOrdersPayload>(EBAY_ORDERS_ACCUMULATED_LS_KEY);
      ...
      try {
        ...
        const res = await fetch(`/api/ebay/orders?${search.toString()}`, { cache: "no-store" });
        const payload = (await res.json()) as OrdersResponse;
        if (!res.ok) {
          const message = payload.error ?? t("ebayOrders.loadFailed");
          setError({
            message,
        ...
        setAllRows((prev) => {
          ...
          writeLocalJsonCache(EBAY_ORDERS_ACCUMULATED_LS_KEY, {
            savedAt: Date.now(),
            items: merged,
        ...
        if (silent) {
          console.warn("[eBay Bestellungen] Hintergrund-Abgleich fehlgeschlagen:", e);
        } else {
          setError({
    ...
    <div className={DASHBOARD_PAGE_SHELL}>
      <div className="space-y-1">
        <div className="flex items-center gap-3">
          ...
          <span className={cn(DASHBOARD_PAGE_TITLE, "text-muted-foreground")}>{t("nav.ebayOrders")}</span>
        </div>
      </div>
      ...
      >
        <div className="flex flex-wrap items-center gap-3 text-xs">
          <span className="rounded-md border border-border/60 bg-background/80 px-2.5 py-1">
            {t("ebayOrders.totalUnits", { count: summary.units })}
          </span>
          <span className="rounded-md border border-border/60 bg-background/80 px-2.5 py-1">
            {t("ebayOrders.sumLabel", { amount: formatAmount(summary.amount, summary.currency) })}
          </span>
          <span className="rounded-md border border-border/60 bg-background/80 px-2.5 py-1">
            {t("ebayOrders.ordersCount", { count: summary.orders })}
          </span>
          {isBackgroundSyncing ? (
            <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
              <Loader2 className="h-3.5 w-3.5 animate-spin" aria-hidden />
              {t("ebayOrders.syncing")}
            </span>
          ) : null}
        </div>
        ...
        <div className="space-y-2 rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-700">
          ...
          {error.missingKeys && error.missingKeys.length > 0 ? (
            <p className="font-mono text-xs text-red-800/90">
              {t("ebayOrders.missingEnvVars", { keys: error.missingKeys.join(", ") })}
            </p>
          ) : null}
        </div>
      ) : null}
      ...
      {isLoading && tableRows.length === 0 && allRows.length === 0 ? (
        <div className="rounded-xl border border-border/50 bg-card/80 p-4 text-sm text-muted-foreground">
          {t("ordersShared.loading", { marketplace: t("nav.ebay") })}
        </div>
      ) : (
        <DataTable
          columns={columns}
