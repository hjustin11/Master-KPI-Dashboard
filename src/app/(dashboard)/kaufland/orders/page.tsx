...
function statusVariantFromRaw(raw: string): "default" | "secondary" | "outline" | "destructive" {
  ...
  if (n.includes("sent") || n.includes("return") || n.includes("received")) return "default";
  if (n.includes("need_to_be_sent") || n.includes("open") || n.includes("pending")) return "secondary";
  return "outline";
}

function useKauflandStatusLabel() {
  const { t } = useTranslation();
  return useCallback(
    (raw: string) => {
      const n = raw.trim().toLowerCase();
      if (!n) return t("kauflandOrders.statusUnknown");
      if (n.includes("cancel")) return t("kauflandOrders.statusCancelled");
      if (n.includes("sent") || n.includes("return") || n.includes("received")) {
        return t("kauflandOrders.statusCompleted");
      }
      if (n.includes("need_to_be_sent") || n.includes("open")) {
        return t("kauflandOrders.statusPending");
      }
      return raw;
    },
    [t]
...
export default function KauflandOrdersPage() {
  ...
  const labelForStatus = useKauflandStatusLabel();
  ...
  const columns = useMemo<Array<ColumnDef<KauflandOrderRow>>>(
    () => [
      {
        accessorKey: "orderId",
        header: t("kauflandOrders.orderId"),
        cell: ({ row }) => <span className="font-medium">{row.original.orderId}</span>,
      },
      {
        accessorKey: "purchaseDate",
        header: t("kauflandOrders.purchaseDate"),
        cell: ({ row }) => (
      ...
      },
      {
        accessorKey: "amount",
        meta: { align: "center" },
        header: () => <div className="block w-full text-center">{t("kauflandOrders.total")}</div>,
        cell: ({ row }) => (
      ...
      },
      {
        accessorKey: "units",
        meta: { align: "center" },
        header: () => <div className="block w-full text-center">{t("kauflandOrders.units")}</div>,
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: t("kauflandOrders.status"),
        cell: ({ row }) => (
  ...
  const loadOrders = useCallback(
    async (nextFrom?: string, nextTo?: string, forceRefresh = false, silent = false) => {
      ...
      try {
        ...
        if (!res.ok) {
          const message = payload.error ?? t("kauflandOrders.loadFailed");
          setError({
            message,
    ...
    <div className={DASHBOARD_PAGE_SHELL}>
      <div className="space-y-1">
        <div className="flex items-center gap-3">
          ...
          <span className={cn(DASHBOARD_PAGE_TITLE, "text-muted-foreground")}>{t("nav.kauflandOrders")}</span>
        </div>
        <p className="text-sm text-muted-foreground">{t("kauflandOrders.subtitle")}</p>
      </div>

      <div className={cn(DASHBOARD_COMPACT_CARD, "flex flex-wrap items-end justify-between gap-3")}>
        <div className="flex flex-wrap items-end gap-3">
          <div className="space-y-1">
            <p className="text-xs font-medium text-muted-foreground">{t("kauflandOrders.period")}</p>
            <Select value={mode} onValueChange={handleModeChange}>
              <SelectTrigger className="w-[220px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="today-yesterday">{t("kauflandOrders.todayYesterday")}</SelectItem>
                <SelectItem value="custom">{t("kauflandOrders.customRange")}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs">
          <span className="rounded-md border border-border/60 bg-background/80 px-2.5 py-1">
            {t("kauflandOrders.totalUnits", { count: summary.units })}
          </span>
          <span className="rounded-md border border-border/60 bg-background/80 px-2.5 py-1">
            {t("kauflandOrders.sumLabel", { amount: formatAmount(summary.amount, summary.currency) })}
          </span>
          <span className="rounded-md border border-border/60 bg-background/80 px-2.5 py-1">
            {t("kauflandOrders.ordersCount", { count: summary.orders })}
          </span>
          {isBackgroundSyncing ? (
            <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
              <Loader2 className="h-3.5 w-3.5 animate-spin" aria-hidden />
              {t("kauflandOrders.syncing")}
            </span>
          ) : null}
        </div>
      </div>
        ...
        <div className="space-y-2 rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-700">
          ...
          {error.missingKeys && error.missingKeys.length > 0 ? (
            <p className="font-mono text-xs text-red-800/90">
              {t("kauflandOrders.missingEnvVars", { keys: error.missingKeys.join(", ") })}
            </p>
          ) : null}
        </div>
      ) : null}
