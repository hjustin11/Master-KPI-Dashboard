...
const FRESSNAPF_ORDERS_ACCUMULATED_LS_KEY = "fressnapf_orders_accumulated_v1";
...
export default function FressnapfOrdersPage() {
  ...
  const [allRows, setAllRows] = useState<FressnapfOrderRow[]>([]);
  ...
  const allRowsRef = useRef<FressnapfOrderRow[]>([]);
  ...
  const loadOrders = useCallback(
    async (nextFrom?: string, nextTo?: string, forceRefresh = false, silent = false) => {
      ...
      if (!forceRefresh && !silent) {
        const parsed = readLocalJsonCache<CachedOrdersPayload>(FRESSNAPF_ORDERS_ACCUMULATED_LS_KEY);
      ...
      try {
        ...
        setAllRows((prev) => {
          ...
          writeLocalJsonCache(FRESSNAPF_ORDERS_ACCUMULATED_LS_KEY, {
            savedAt: Date.now(),
            items: merged,
    ...
    <div className={DASHBOARD_PAGE_SHELL}>
      ...
      {isLoading && tableRows.length === 0 && allRows.length === 0 ? (
        <div className="rounded-xl border border-border/50 bg-card/80 p-4 text-sm text-muted-foreground">
          {t("ordersShared.loading", { marketplace: t("nav.fressnapf") })}
        </div>
      ) : (
        <DataTable
          columns={columns}
