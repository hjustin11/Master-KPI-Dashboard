...
function statusVariantFromRaw(raw: string): "default" | "secondary" | "outline" | "destructive" {
  ...
  if (n.includes("ship") || n.includes("deliver") || n.includes("complete")) return "default";
  if (n.includes("pend") || n.includes("open") || n.includes("process")) return "secondary";
  return "outline";
}

function useOttoStatusLabel() {
  const { t } = useTranslation();
  return useCallback(
    (raw: string) => {
      const n = raw.trim().toLowerCase();
      if (!n) return t("ottoOrders.statusUnknown");
      if (n.includes("cancel")) return t("ottoOrders.statusCancelled");
      if (n.includes("ship") || n.includes("deliver") || n.includes("complete")) {
        return t("ottoOrders.statusCompleted");
      }
      if (n.includes("pend") || n.includes("open") || n.includes("process")) {
        return t("ottoOrders.statusPending");
      }
      return raw;
    },
    [t]
...
export default function OttoOrdersPage() {
  ...
  const labelForStatus = useOttoStatusLabel();
  ...
  const [rows, setRows] = useState<OttoOrderRow[]>([]);
  ...
  const columns = useMemo<Array<ColumnDef<OttoOrderRow>>>(
    () => [
      {
        accessorKey: "orderId",
      ...
      },
      {
        ...
        header: t("ottoOrders.purchaseDate"),
        cell: ({ row }) => (
      ...
      },
      {
        accessorKey: "amount",
        meta: { align: "center" },
        header: () => <div className="block w-full text-center">{t("ottoOrders.total")}</div>,
        cell: ({ row }) => (
      ...
      },
      {
        accessorKey: "units",
        meta: { align: "center" },
        header: () => <div className="block w-full text-center">{t("ottoOrders.units")}</div>,
        cell: ({ row }) => (
  ...
  const loadOrders = useCallback(
    async (nextFrom?: string, nextTo?: string, forceRefresh = false, silent = false) => {
      ...
      const cacheKey = `otto_orders_cache_v1:${f}:${rangeTo}`;
      let hadCache = false;
      ...
      try {
        ...
        const res = await fetch(`/api/otto/orders?${search.toString()}`, { cache: "no-store" });
        const payload = (await res.json()) as OrdersResponse;
        if (!res.ok) {
          const message = payload.error ?? t("ottoOrders.loadFailed");
          setError({
            message,
        ...
        if (silent) {
          console.warn("[Otto Bestellungen] Hintergrund-Abgleich fehlgeschlagen:", e);
        } else {
          setRows([]);
          setError({
    ...
    <div className={DASHBOARD_PAGE_SHELL}>
      <div className="space-y-1">
        <div className="flex items-center gap-3">
          <span className="relative block h-10 w-[120px] shrink-0">
            <Image
          ...
          <span className={cn(DASHBOARD_PAGE_TITLE, "text-muted-foreground")}>{t("nav.ottoOrders")}</span>
        </div>
      </div>
      ...
      >
        <div className="flex flex-wrap items-center gap-3 text-xs">
          <span className="rounded-md border border-border/60 bg-background/80 px-2.5 py-1">
            {t("ottoOrders.totalUnits", { count: summary.units })}
          </span>
          <span className="rounded-md border border-border/60 bg-background/80 px-2.5 py-1">
            {t("ottoOrders.sumLabel", { amount: formatAmount(summary.amount, summary.currency) })}
          </span>
          <span className="rounded-md border border-border/60 bg-background/80 px-2.5 py-1">
            {t("ottoOrders.ordersCount", { count: summary.orders })}
          </span>
          {isBackgroundSyncing ? (
            <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
              <Loader2 className="h-3.5 w-3.5 animate-spin" aria-hidden />
              {t("ottoOrders.syncing")}
            </span>
          ) : null}
        </div>
        ...
        <div className="ml-auto flex flex-wrap items-center justify-end gap-2">
          <div className="flex items-center gap-2">
            <p className="text-xs font-medium text-muted-foreground">{t("ottoOrders.period")}</p>
            <Select value={mode} onValueChange={handleModeChange}>
              <SelectTrigger className="w-[220px]">
                <SelectValue>
                  {mode === "today-yesterday" ? t("ottoOrders.todayYesterday") : t("ottoOrders.customRange")}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="today-yesterday">{t("ottoOrders.todayYesterday")}</SelectItem>
                <SelectItem value="custom">{t("ottoOrders.customRange")}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        ...
        <div className="space-y-2 rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-700">
          ...
          {error.missingKeys && error.missingKeys.length > 0 ? (
            <p className="font-mono text-xs text-red-800/90">
              {t("ottoOrders.missingEnvVars", { keys: error.missingKeys.join(", ") })}
            </p>
          ) : null}
        </div>
      ) : null}
